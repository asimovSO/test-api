...
## Присутствие (Cache-фасад) — в процессе (2026-08-08)

Спроектировали, писать код ещё не начали. `.env` настроен (`REDIS_CLIENT=phpredis`),
но решили работать через `Cache`-фасад, а не через `Redis`-фасад напрямую.

**Пересмотр решения (было — ZSET, стало — точечные TTL-ключи).** Изначально
спроектировали один sorted set `users:online` на всех через `Redis`-фасад — под задачу
«дать список всех, кто онлайн». Но реальная точка использования — `onlineStatuses(array
$userIds)` перед раздачей списка бесед, т.е. проверка статуса для **конкретного известного
набора id** (участников), а не глобальный запрос «кто вообще онлайн». Под такую задачу ZSET
избыточен: SCAN-проблема, ради которой его выбрали, тут не возникает вообще.

Из-за этого пересмотра поменялся и слой хранения: `Redis`-фасад отдаёт команды Redis
один в один (`ZADD`, `ZMSCORE`...) и требует sorted set. `Cache`-фасад — абстракция
ключ-значение, ZSET через него не сделать, зато для точечных TTL-ключей он ровно то, что
нужно и не привязывает код к конкретному драйверу.

**Схема хранения:** по ключу на юзера, `presence:{userId}`, значение неважно (например
`true`), TTL ~40 сек (30-секундный heartbeat + запас, чтобы одна задержавшаяся сеть не
сбрасывала в оффлайн).
- `Cache::put("presence:{$userId}", true, 40)` в heartbeat.
- Батч-проверка статуса нескольких юзеров разом — `Cache::many([...])`, а не `Cache::get()`
  в цикле (тот же принцип N+1, что и с БД).
- Отдельная очистка (`cleanup`) не нужна: TTL встроен в сам ключ, драйвер удалит его сам.
  Планировщик под presence в итоге не понадобится.
- Явно отклонили: колонку `is_online` в таблице `users`. Heartbeat раз в 30 сек означал бы
  запись в основную БД с той же частотой, плюс колонке без TTL нечем «протухать» самой —
  нужен был бы отдельный джоб, гасящий флаг. Именно ради этого presence и не хранят
  в основной БД.

**Где будет жить логика:** `app/Services/PresenceService.php` (по конвенции проекта —
бизнес-логика не в контроллере), два метода:
- `heartbeat(int $userId): void` — `Cache::put`, дёргается из `CheckUserIsOnline::__invoke()`.
- `onlineStatuses(array $userIds): array` — `Cache::many`, возвращает `[id => bool]`;
  дёргается из контроллера списка бесед перед раздачей `ConversationResource`, чтобы
  на каждой `User`-модели проставить `$user->online = ...` (доп. атрибут на модели —
  штатный способ Laravel передать вычисленное значение в ресурс, в БД не пишется)
  до того, как модель уйдёт в `UserResource`.

**Следующий шаг:** пользователь сам пишет `PresenceService::heartbeat()` и показывает
на ревью.
