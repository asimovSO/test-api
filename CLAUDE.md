...
- **Непочиненный баг:** колбэк канала возвращает `[new UserResource($user)]` — массив из
  одного элемента, а не сам объект. На выходе `user_info` в ответе `/broadcasting/auth`
  получается `[{"id":2,"name":"Alice"}]` вместо `{"id":2,"name":"Alice"}`. Правильно —
  `return new UserResource($user);` без обёртки в `[...]` (Laravel сам оборачивает
  возвращаемое значение в `user_info`, повторно оборачивать не нужно). Не исправлено —
  фронт (см. `API.md`) знает про это как про временную шероховатость.
- **Ещё не убрано:** мёртвый импорт `use App\Http\Controllers\CheckUserIsOnline;` в
  `routes/api.php` — контроллер удалён, импорт остался (не роняет приложение, просто мусор).
...
1. Баг с обёрткой `[UserResource]` в presence-канале (см. выше).
2. Мёртвый импорт `CheckUserIsOnline` в `routes/api.php`.
3. `PresenceService.php` — код-мертвец, можно удалять вместе с остальными Redis-присутствие
